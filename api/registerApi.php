...
if (empty($_POST["name"]) || empty($_POST["surname"]) || !filter_var($_POST["email"], FILTER_VALIDATE_EMAIL) ||
    ...
    header("Location: https://wt98.fei.stuba.sk/authentication/index.php/?chyba=1");
} else {
    ...
    if (strcmp($password, $passwordVerify) != 0 || strlen($password) > 16) {
        header("Location: https://wt98.fei.stuba.sk/authentication/index.php/?chyba=2");
    } else {
        ...
        if (!is_null($user = $userController->getUserByEmail($email))) {
            header("Location: https://wt98.fei.stuba.sk/authentication/index.php/?chyba=3");
        }
