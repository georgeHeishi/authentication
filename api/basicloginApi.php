...
if (!filter_var($_POST["email"], FILTER_VALIDATE_EMAIL) || empty($_POST["password"]) || empty($_POST["code"])) {
    header("Location: https://wt98.fei.stuba.sk/authentication/index.php/?loginError=1");
} else {
    $email = $_POST["email"];
    ...
    if (is_null($user) || !checkSecret($code, $user->getSecret())) {
        header("Location: https://wt98.fei.stuba.sk/authentication/index.php/?loginError=2");
    } else {
